...
sync_secret_env() {
  ...
  # Source checkout for scanners that need a working tree (gitleaks). Enables the
  # Code & Dependency Safety pillar. Clones are depth-1, HTTPS-only and always
  # removed; the size cap bounds what is retained and the timeout bounds the peak.
  upsert_secret_env "AI_SUPPLY_CHAIN_TRUST_SCANNER_SOURCE_CHECKOUT" "${AI_SUPPLY_CHAIN_TRUST_SCANNER_SOURCE_CHECKOUT:-1}"
  upsert_secret_env "AI_SUPPLY_CHAIN_TRUST_SCANNER_CHECKOUT_MAX_BYTES" "${AI_SUPPLY_CHAIN_TRUST_SCANNER_CHECKOUT_MAX_BYTES:-268435456}"
  upsert_secret_env "AI_SUPPLY_CHAIN_TRUST_SCANNER_CHECKOUT_TIMEOUT_SECONDS" "${AI_SUPPLY_CHAIN_TRUST_SCANNER_CHECKOUT_TIMEOUT_SECONDS:-120}"
